...
- **تسجيل دفعات باركود جديدة (v4.2)** — وضع مستقل تمامًا عن وضع البيع (له الأولوية دائمًا
  ...
  يعرض العدّاد فوق كل الصفحات)، ثم تُربط كل الباركودات باسم منتج رئيسي واحد — جديد أو
  إضافة لموجود بنفس الاسم بالضبط — مع إمكانية تقسيمها لعدة "مجموعات أسعار" (كل مجموعة
  سعر جملة/بيع مستقل)، وكمية إجمالية واحدة تُضاف لرصيد المنتج. عند البيع لاحقًا، كل باركود
  يستخدم سعر مجموعته الخاصة تلقائيًا حتى لو كانت كلها تحت نفس اسم المنتج. يمنع تسجيل أي
  باركود مكرر (سواء من دفعة سابقة أو من نفس الدفعة الحالية) مع تنبيه واضح.
